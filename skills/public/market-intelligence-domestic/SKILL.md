...
3. **SQL 字符串永不回显**(数据明细段 / MRTF 段都不允许)
4. **`地理区域` vs `销售/运营单位` 严格区分**(集团/公司名 → 销售单位,不是 region)
5. **数据段 S0**:严格复制 `executeSqlToolCallback` 返回行,禁编造日期 / 省份 / 销售单位 / 数值
...
11. **本平台数据末尾 `[数据]` 标识**
12. **首句即结论 + ≥ 3 处具体数值**
13. **禁:SQL / 表名 / `listAgents*` / `nl2Sql*` / `executeSql*` / `agentId` / `mcp_*` / `team2` / `sub-2-*` / `/mnt/skills/...` 路径 / `read_file(...)` 调用语法 等内部组件名**
