# Outputs → MinIO 集成实施计划

> **For Claude：** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**目标：** 当 `present_files` 被调用时,将 agent 产生的输出文件同步到 MinIO；当本地文件缺失时,在 artifacts 接口中回退到 MinIO。

**架构：** `present_files` 工具在完成路径归一化后立即将每个文件上传到 MinIO（push）。artifacts 接口优先读取本地磁盘,若本地不存在再回退到 MinIO（pull）。这是 outputs 在 Python 层面唯一干净的钩子；由 bash 直接写入 workspace 的文件不在本次范围内。

**技术栈：** Python 3.12、`minio>=7.2`、FastAPI、pytest、`unittest.mock`

---

### 任务 1：在 `present_file_tool` 中加入 MinIO push

**涉及文件：**
- 修改：`backend/packages/harness/deerflow/tools/builtins/present_file_tool.py`
- 测试：`backend/tests/test_present_file_tool_core_logic.py`

---

**Step 1：编写会失败的测试**

在 `backend/tests/test_present_file_tool_core_logic.py` 末尾追加：
...
预期：`FAILED` —— `AttributeError: module has no attribute 'get_uploads_config'`

---

**Step 3：在 `present_file_tool.py` 中实现 MinIO push**

用以下内容替换 `backend/packages/harness/deerflow/tools/builtins/present_file_tool.py` 的全部内容：
...
### 任务 2：在 artifacts 接口中加入 MinIO 兜底

**涉及文件：**
- 修改：`backend/app/gateway/routers/artifacts.py`
- 测试：`backend/tests/test_artifacts_router.py`

---

**Step 1：编写会失败的测试**

在 `backend/tests/test_artifacts_router.py` 末尾追加：
...
预期：`FAILED` —— 在 `get_uploads_config` 处抛出 `ImportError` 或 `AttributeError`

---

**Step 3：在 `artifacts.py` 中实现 MinIO 兜底**

在 `backend/app/gateway/routers/artifacts.py` 顶部现有 import 之后新增：

```python
from deerflow.config.uploads_config import get_uploads_config
```

将 `if not actual_path.exists():` 所在的代码块(当前仅是简单返回 404)替换为：
...
原先 `if not actual_path.exists(): raise HTTPException(status_code=404 ...)` 的那一行会**被上面这段代码块替换**。
