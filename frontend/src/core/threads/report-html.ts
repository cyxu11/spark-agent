...
export function renderReportHtml(
  ...
  const { body, toc } = renderBody(markdownContent || "_无内容_");
  const tocHtml =
    toc.length > 1
...
<body>
  ...
  <div class="report-shell">
    <header class="report-cover">
      ...
      <div class="meta">
        <span><strong>导出时间</strong> ${escapeHtml(exportedAt)}</span>
        <span><strong>会话 ID</strong> ${escapeHtml(meta.threadId.slice(0, 8))}</span>
